...
const serverUrl = 'https://your-server.com/api'; // Replace with your server URL
...
async function initializeUser() {
    if (window.Telegram && window.Telegram.WebApp) {
        ...
        if (!userId) {
            showNotification('Помилка: Не вдалося отримати Telegram ID');
            return;
        }
        await loadGame();
    } else {
        showNotification('Помилка: Telegram Web App недоступний');
    }
}

async function loadGame() {
    try {
        const response = await fetch(`${serverUrl}/load/${userId}`, {
            method: 'GET',
